...
(function (exports) {
  ...
  // ---------------------------------------------------------------------------
  // Host liveness / status-dot classification
  //
  // The dot color must reflect whether a host is HEALTHY, not merely whether the
  // single most-recent status snapshot happened to catch the runner service in
  // its 'running' sample. Under boundary-only publication (the current fleet
  // mode) a snapshot is taken AT a task boundary, and the runner can legitimately
  // report state 'stopped' in that instant while a task is actively in flight
  // (boundary 'starting', local queue non-empty) or while the host sits healthy
  // and idle between tasks (boundary 'completed'/'failed', queue empty).
  //
  // Rules, in order:
  //   1. No payload at all         -> 'unreachable' (grey).
  //   2. Boundary publisher active AND the host shows active work
  //      (boundary 'starting' OR local queue depth > 0):
  //        - treat as working; dot is green ('running') unless the snapshot has
  //          gone truly stale, using the WIDE window (a real sweep cell can run
  //          for hours between boundary publishes).
  //   3. Boundary publisher active AND idle (terminal boundary, empty queue):
  //        - healthy idle; dot is green unless stale under the wide window. An
  //          idle host that simply hasn't hit a new boundary is NOT a fault.
  //   4. Periodic publisher (legacy) OR migration still required:
  //        - the old NARROW window applies (updates are expected every minute),
  //          and the reported runner state is trusted directly.
  //
  // Staleness always wins over a healthy classification: a genuinely stale
  // snapshot escalates to 'stale' (amber) then 'stopped' (red) regardless of the
  // work signals, because we can no longer trust any of them.
  //
  // Returns { dotClass, staleWarn, staleCrit, stale, staleReason, boundaryOnly,
  //           boundaryTaskRunning, working }.
  // ---------------------------------------------------------------------------
  var LIVENESS_WARN_WIDE = 7200;    // 2h  — boundary-only publisher
  var LIVENESS_CRIT_WIDE = 21600;   // 6h  — boundary-only publisher
  var LIVENESS_WARN_NARROW = 300;   // 5m  — periodic publisher
  var LIVENESS_CRIT_NARROW = 1800;  // 30m — periodic publisher

  function computeHostLiveness(data, nowMs) {
    if (!data) {
      return {
        dotClass: 'unreachable', staleWarn: false, staleCrit: false, stale: false,
        staleReason: '', boundaryOnly: false, boundaryTaskRunning: false, working: false,
      };
    }

    var now = nowMs == null ? Date.now() : nowMs;
    var reportedState = (data.runner && data.runner.state) || 'unreachable';
    var ageSec = data.timestamp ? (now - new Date(data.timestamp).getTime()) / 1000 : Infinity;

    var iso = data.measurement_isolation || {};
    var boundaryOnly = iso.boundary_publisher_active === true && iso.status_timer_migration_required === false;
    var boundaryState = data.boundary && data.boundary.state;
    var queueDepth = (data.queue && data.queue.depth) || 0;

    var boundaryTaskRunning = boundaryOnly && boundaryState === 'starting';
    // A boundary-only host is "working" whenever a task is starting OR there is
    // queued local work — the runner-service state sampled at a boundary is not
    // authoritative in that instant.
    var working = boundaryOnly && (boundaryTaskRunning || queueDepth > 0);

    // Wide window applies to any boundary-only host (working or healthy-idle);
    // narrow window applies to the legacy periodic publisher.
    var warnAfter = boundaryOnly ? LIVENESS_WARN_WIDE : LIVENESS_WARN_NARROW;
    var critAfter = boundaryOnly ? LIVENESS_CRIT_WIDE : LIVENESS_CRIT_NARROW;

    var staleWarn = ageSec > warnAfter && ageSec <= critAfter;
    var staleCrit = ageSec > critAfter;
    var stale = staleWarn || staleCrit;
    ...
    if (staleCrit) {
      ...
      // Fresh boundary-only snapshot: the host is healthy whether working or
      // idle. Do not paint it red just because the boundary snapshot caught the
      // runner service reporting 'stopped'.
      dotClass = 'running';
    } else {
      // Legacy periodic publisher: trust the reported runner state directly.
      dotClass = reportedState;
    }

    var staleReason = '';
    if (stale) {
      staleReason = boundaryTaskRunning
        ? 'no completion boundary received (task may be stuck)'
        : boundaryOnly
          ? 'no boundary update received (runner may be stopped)'
    ...
    return {
      ...
      boundaryTaskRunning: boundaryTaskRunning,
      working: working,
    };
  }
  ...
  exports.computeHostLiveness = computeHostLiveness;
