...
(function (exports) {
  ...
  function createRecheckScheduler(opts) {
    ...
    // Map<compositeKey, timerId> where compositeKey = hostId + ':' + boundaryKey
    var pending = {};

    function schedule(hostId, boundaryKey) {
      var compositeKey = hostId + '::' + boundaryKey;

      // Already scheduled for this exact boundary — deduplicate.
      if (pending[compositeKey] != null) return false;
