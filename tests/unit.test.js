...
describe('HTML structural integrity', () => {
  ...
  it('status.html has no progress bar or ETA (boundary-only)', () => {
    const html = fs.readFileSync(path.join(root, 'status.html'), 'utf8');
    assert.ok(!html.includes('progress-bar'), 'progress bar must not be present');
    assert.ok(!html.includes('progress_pct'), 'progress percentage must not be present');
    assert.ok(!html.includes('ETA '), 'ETA must not be present');
  });
